...
impl<T : Widget, A : Widget> Widget for Combined<T,A> {
    fn render(&self, window: &mut Window, at : &mut u32) { 
        window.flush().unwrap();
        let surface = Surface::new(self.location.size.x as u32, self.location.size.y as u32).unwrap();
        surface.render_to(
            window,
            |v| {
        ...
        *at+=1;
        window.draw_ex(&self.location,Img(surface.image()),Transform::IDENTITY,*at);
     }
    fn contains(&self, point: Vector) -> bool { 
        let new_point : Vector = (point.x - self.location.pos.x , point.y -self.location.pos.y).into();
