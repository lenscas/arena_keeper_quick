...
use quicksilver::{combinators::result, Future};
...
{
    Asset::new(Font::load(font).and_then(move |font| {
        let style = FontStyle::new(size, color);
        fun(font, style)
    }))
}
pub fn load_from<F: 'static, T: 'static>(
    font: String,
    size: f32,
    color: Color,
    builder: F,
...
where
    F: Finalize<to = T>,
{
    load(font, size, color, |font, style| {
        result(builder.to_state(font, style))
    })
}
