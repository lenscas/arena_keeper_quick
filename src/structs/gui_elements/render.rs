...
impl<A> Render<A> {
	pub fn render<F>(&mut self, window : &mut Window, state : &mut UiState, mut to_render : F,) -> Result<()>
	where
		F : FnMut(DrawContext<ImmiRender>, &mut UiState, &mut A) -> (),
	{
		...
		self.asset.execute(|(font, button)| {
			...
			to_style(&mut Box::new(&mut render),create_immi_ctx(ui_status, &mut ui_render));
			Ok(())
		})
	}
}