...
impl Characters {
    ...
    fn update_paralel(&mut self, grid: &Field, mods: &ModulesContainer) {
        self.characters
            .iter_mut()
            .for_each(|v| v.update_par(grid, mods.all_tiles));
    }
    #[cfg(not(target_arch = "wasm32"))]
    fn update_paralel(&mut self, grid: &Field, mods: &ModulesContainer) {
        use rayon::prelude::*;
        let tiles = &mods.all_tiles;
        let species = &mods.all_species;
        self.characters
            .par_iter_mut()
            .for_each(|v| v.update_par(grid, species, tiles));
    }
    pub fn update(&mut self, grid: &mut Field, mods: &ModulesContainer) {
        self.update_paralel(grid, mods);
        self.characters
            .iter_mut()
            .for_each(|v| v.update(grid, &mods.all_species, &mods.all_tiles));
    }
